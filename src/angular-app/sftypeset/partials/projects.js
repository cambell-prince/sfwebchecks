...
angular.module(
	...
	.controller('ProjectsCtrl', ['$scope', 'projectService', 'sessionService', 'breadcrumbService', 'linkService', 
		...
		// New Project
		$scope.projectTypes = [
	        {key: 'check',   label: 'Community Checking'},
	        {key: 'typeset', label: 'Publishing'}
        ];
		$scope.newProject = {};
		$scope.newProject.type = $scope.projectTypes[1].key;
		...
		$scope.queryProjectsForUser = function() {
			...
			projectService.list(projectService.TYPE_ALL, function(result) {
				if (result.ok) {
		...
		$scope.addProject = function() {
			console.log("addProject()", $scope.newProject);
			var model = {};
			model.id = '';
			model.projectname = $scope.newProject.projectname;
			model.type = $scope.newProject.type;
		...
		$scope.projectUrl = function(project) {
			if (project.type == projectService.TYPE_TYPESET()) {
				return '/app/sftypeset#/project/' + project.id;
			} else {
				return '/app/sfchecks#/project/' + project.id;
			}
		}; 
